...
const SentinelHubWmsService_1 = require("./SentinelHubWmsService");
...
class FindStructure {
    ...
    getInvalidPixels() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const bbox = SentinelHubWmsService_1.SentinelHubWms.latLngToXYTool(this.shape)[0];
            const imgRaw = yield SentinelHubWmsService_1.SentinelHubWms.getImage(this.uuid, bbox.getBobxConnors(), { date: this.dateFromShape, layers: [WMSParameters_1.WMSParameters.Sentinel_2.NDVI] });
            const img = yield this.RawToImage(imgRaw.blob);
            const paintedImage = yield this.paintInvalidInDangerZone(img, returnGeoJson);
            const shape = yield SentinelHubWmsService_1.SentinelHubWms.createShapeAsImage(this.shape, paintedImage, bbox);
    ...
    getDangerZoneByDate(date) {
        return __awaiter(this, void 0, void 0, function* () {
            const bbox = SentinelHubWmsService_1.SentinelHubWms.latLngToXYTool(this.shape)[0];
            const imgRaw = yield SentinelHubWmsService_1.SentinelHubWms.getImage(this.uuid, bbox.getBobxConnors(), { date, layers: [WMSParameters_1.WMSParameters.Sentinel_2.NDVI] });
            const img = yield this.RawToImage(imgRaw.blob);
            return SentinelHubWmsService_1.SentinelHubWms.getDangerZone(this.shape, img);
        });
    }
    RawToImage(blob) {
            ...
            Array.from(Array(colorManager.canvas.width).keys()).forEach((x) => {
                Array.from(Array(colorManager.canvas.height).keys()).forEach((y) => {
                    ...
                    if (data[0] > 100 && data[2] < 72 && data[1] < 100) {
                        const latLong = SentinelHubWmsService_1.SentinelHubWms.latLngToXYTool(shape)[0];
