...
export class FindStructure {
    ...
    public async getInvalidPixels(): Promise<{ img: string, LatLng: [number[], number[]], link: string }> {
        ...
        const imgRaw = await SentinelHubWms.getImage(this.uuid, bbox.getBobxConnors(), { date: this.dateFromShape, layers: [WMSParameters.Sentinel_2.NDVI] });
    ...
    private async  getDangerZoneByDate(date: Date) {
        const bbox = SentinelHubWms.latLngToXYTool(this.shape)[0];
        const imgRaw = await SentinelHubWms.getImage(this.uuid, bbox.getBobxConnors(), { date, layers: [WMSParameters.Sentinel_2.NDVI] });
        const img = await this.RawToImage(imgRaw.blob);
